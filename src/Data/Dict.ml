...
module Make(K : Comparable.Base) : sig
  ...
  let values map =
    Stdlib.List.rev (fold map ~init:[] ~f:(fun ~key ~data acc -> data::acc))

  let to_assoc map =
    Stdlib.List.rev (fold map ~init:[] ~f:(fun ~key ~data acc -> (key, data)::acc))

  let add_assoc m l =
    Stdlib.List.fold_left (fun m (k,v) -> add k v m) m l
  ...
  let aggregate_list ~by:f ~init ~f:op l =
    Stdlib.List.fold_left l ~init:empty ~f:(fun acc elem ->
...
(* module Make(K : Stdlib.Map.OrderedType) : S *)
(*   with type 'a t = 'a Stdlib.Map.Make(K).t *)
(*    and type key = K.t = *)
(* struct *)
