...
module Old = struct
  ...
  let filter_map f self =
    self
    |> fold_left (fun acc e -> f e::acc) []
    |> fold_left (fun acc e -> match e with None -> acc | Some x -> x :: acc) []
  ...
  let join sep l =
    ...
    | hd :: tl ->
      ...
      List.iter (fun s -> incr str_num; res_len := !res_len + String.len s) l;
