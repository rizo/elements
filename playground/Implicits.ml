...
implicit module List {Item : sig type t end} = struct
  type t = Item.t list
  type cursor = t
  type item = Item.t
...
implicit module String_Reducible = struct
  type t = string
  type item = string
...
implicit module String_Reducible_Char = struct
  type t = string
  type item = char
  type accumulator = string
...
let iter {Self : Iterable} (self : Self.t) =
  { init = (fun () -> Self.cursor self);
    next = (fun yield r s -> Self.next self yield r s);
    free = ignore }


let into {Self : Reducible} (self : Self.t) iter =
